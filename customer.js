...
function renderAccounts(){
    ...
    for (let account of list_accounts){
        ...
        <tr style="color: grey;">
            ...
            <td>
                <a onclick="blockAccount(${account.id})" class="edit" style="cursor: pointer">${!account.is_block ? "Block":"Active"}</a>
                <a onclick="deleteAccountByID(${account.id})" class="delete" style="cursor: pointer">Delete</a>
            </td>
        </tr> 
        `
    }
...
function deleteAccountChecked(){
    const p = confirm("Bạn có muốn xóa tài khoản đã chọn?")
    if (p){
        //lấy ra các checkbox
